...
class SMS:
    def __init__(self):
        ...
        # self.sets()
        # self.vars_ex()
        # self.params_ex()    # add E2X and X2E conversion intercept term
        ...
        # self.aux_Cons_ex()  # relations defining auxiliary variables
        ...
        # self.E2X_Cons_ex()      # E2X relations, suitable for modeling no-liner process
        # self.stor_Cons_ex()  # storage device relations
        # self.X2E_Cons_ex()      # X2E relations, suitable for modeling no-liner process
    ...
    # var, param and relations for ESSs including CAESS
    def vars_ex(self):
        m = self.m
        # define variables needed for demonstrating decorators
        # 1) decision variables
        ...
        m.xVol = pe.Var(m.S_, m.Th, within=pe.NonNegativeReals)   # energy form stored in s-th storage device.
        m.xOuts = pe.Var(m.S_, m.T, within=pe.NonNegativeReals)     # energy form outflow from s-th storage device.
        m.xInx = pe.Var(m.X_, m.T, within=pe.NonNegativeReals)      # total energy form inflow to x-th X2E converter.
        m.xInxx = pe.Var(m.X_, m.T, within=pe.NonNegativeReals)    # x inflow to X2E for conversion above min cap.
        m.eOutx = pe.Var(m.X_, m.T, within=pe.NonNegativeReals)  # energy form outflow from x-th X2E converter.
        m.eB = pe.Var(m.T, within=pe.NonNegativeReals)        # electricity purchase on the market, [MWh].
        ...
        m.sCap = pe.Var(m.S_, within=pe.NonNegativeReals)  # total capacity of s-th type of storage device.
        m.xCap = pe.Var(m.X_, within=pe.NonNegativeReals)  # total capacity of x-th type of X2E converter.
        # m.e2xMxOut = pe.Var(m.E_, within=pe.NonNegativeReals)  # lower capacity of e-th type of E2X converter.
        ...
        m.ye = pe.Var(m.E_, m.T, within=pe.Binary)               # activate var for E2X conversion
        m.ys = pe.Var(m.S_, m.T, within=pe.Binary)               # activate var for storage
        m.yx = pe.Var(m.X_, m.T, within=pe.Binary)               # activate var for X2E conversion
    ...
    def params_ex(self):
        ...
        m.e2x = pe.Param(m.E_, domain=pe.NonNegativeReals)     # E2X conversion factor, [xxx/MWh].
        m.e2xInt = pe.Param(m.E_, domain=pe.Reals)             # E2X conversion intercept term [MWh].
        # m.mxe2xOut = pe.Param(m.E_, domain=pe.NonNegativeReals)     # maximum x output from E2X converter.
        m.e2s = pe.Param(m.S_, domain=pe.NonNegativeReals)     # electricity consumption factor, [MWh/xxx].
        m.e2sInt = pe.Param(m.S_, domain=pe.NonNegativeReals)     # electricity consumption intercept term, [MWh].
        m.mxsIn = pe.Param(m.S_, domain=pe.NonNegativeReals)   # maximum energy form inflow to the storage device.
        m.mxsOut = pe.Param(m.S_, domain=pe.NonNegativeReals)  # maximum energy form from the storage device.
        m.sini = pe.Param(m.S_, domain=pe.NonNegativeReals)    # percentage of initial energy form in device.
        m.xRes = pe.Param(m.S_, domain=pe.NonNegativeReals)    # energy form retention rate.
        ...
        m.xDis = pe.Param(m.S_, domain=pe.NonNegativeReals)    # discharging efficiency.
        m.x2e = pe.Param(m.X_, domain=pe.NonNegativeReals)     # X2E conversion factor, [MWh/xxx].
        m.x2eInt = pe.Param(m.X_, domain=pe.Reals)             # X2E conversion intercept term [MWh].
        m.xBec = pe.Param(m.X_, domain=pe.NonNegativeReals)  # Baseline energy consumption of X2E converter.

        m.Mmax = pe.Param(initialize=1.0e3)                             # big M for activate conversion constraints
        m.mmin = pe.Param(initialize=1.0e-4)                          # small m for activate conversion constraints
    ...
    def aux_Cons_ex(self):
        ...
        # @m.Constraint(m.E_)  # lower capacity of e-type E2X converter
        # def e2xMxOutC(mx, r, e):
        #     return mx.e2xMxOut[r, e] == mx.mxe2xOut[r, e] * mx.eNum[r, e]
    ...
    def E2X_Cons_ex(self):
        ...
        @m.Constraint(m.E_, m.T)  # eIne upper, if eIne = 0, xOute = 0
        def eIneUp1(mx, r, e, t):
            return mx.eIne[r, e, t] <= mx.Mmax * mx.ye[r, e, t]

        @m.Constraint(m.E_, m.T)  # eIne low, if eIne = 0, xOute = 0
        def eIneLow1(mx, r, e, t):
            return mx.eIne[r, e, t] >= mx.mmin * mx.ye[r, e, t]

        @m.Constraint(m.E_, m.T)  # x converted from e-th E2X converter
        def xOuteUp1(mx, r, e, t):
            return mx.xOute[r, e, t] <= mx.e2x[r, e] * mx.eIne[r, e, t] + mx.e2xInt[r, e]

        @m.Constraint(m.E_, m.T)  # x converted from e-th E2X converter
        def xOuteUp2(mx, r, e, t):
            return mx.xOute[r, e, t] <= mx.Mmax * mx.ye[r, e, t]

        @m.Constraint(m.E_, m.T)  # x converted from e-th E2X converter
        def xOuteLow1(mx, r, e, t):
            return mx.xOute[r, e, t] >= (
                    mx.e2x[r, e] * mx.eIne[r, e, t] + mx.e2xInt[r, e]
                    - mx.Mmax * (1 - mx.ye[r, e, t])
            )
        ...
        # @m.Constraint(m.E_, m.T)  # x converted from e-th E2X converter
        # def xOuteUp(mx, r, e, t):
        #     return mx.xOute[r, e, t] <= mx.intv * mx.e2xMxOut[r, e]

    def stor_Cons_ex(self):
        m = self.m
        ...
        @m.Constraint(m.S_, m.T)     # xIns setting, link xIns = 0, ys = 0
        def xIns_to_ys_up(mx, r, s, t):
            return mx.xIns[r, s, t] <= mx.Mmax * mx.ys[r, s, t]

        @m.Constraint(m.S_, m.T)     # xIns setting, link xIns = 0, ys = 0
        def xIns_to_ys_low(mx, r, s, t):
            return mx.xIns[r, s, t] >= mx.mmin * mx.ys[r, s, t]

        @m.Constraint(m.S_, m.T)  # additional electricity used for storing X in a storage device
        def eInsUp1(mx, r, s, t):
            return mx.eIns[r, s, t] <= mx.e2s[r, s] * mx.xIns[r, s, t] + mx.e2sInt[r, s]

        @m.Constraint(m.S_, m.T)  # if xIns = 0, force eIns = 0
        def eInsUp2(mx, r, s, t):
            return mx.eIns[r, s, t] <= mx.Mmax * mx.ys[r, s, t]

        @m.Constraint(m.S_, m.T)    # eInsUp2 and eOutsLow1: force if xins = 0, then eIns = 0.
        def eOutsLow1(mx, r, s, t):
            return (mx.eIns[r, s, t] >= (
                    mx.e2s[r, s] * mx.xIns[r, s, t] + mx.e2sInt[r, s]
                    - mx.Mmax * (1 - mx.ys[r, s, t]))
                    )
    ...
    def X2E_Cons_ex(self):
        ...
        @m.Constraint(m.X_, m.T)        # x flow split to baseline energy consumption and variable consumption
        def xInxC(mx, r, x, t):
            return mx.xInx[r, x, t] == mx.xInxx[r, x, t] + mx.xBec[r, x] * mx.yx[r, x, t]

        @m.Constraint(m.X_, m.T)        # link xInx to yx, yx = 0, xInx = 0
        def xInx_to_yx(mx, r, x, t):
            return mx.xInxx[r, x, t] <= mx.Mmax * mx.yx[r, x, t]

        @m.Constraint(m.X_, m.T)  # electricity outflow from a X2E converter
        def eOutxC(mx, r, x, t):
            return mx.eOutx[r, x, t] <= mx.x2e[r, x] * mx.xInxx[r, x, t] + mx.x2eInt[r, x]

        @m.Constraint(m.X_, m.T)  # upper constraint of electricity outflow from a X2E converter
        def eOutxUp1(mx, r, x, t):
            return mx.eOutx[r, x, t] <= mx.intv * mx.xCap[r, x]

        @m.Constraint(m.X_, m.T)    # if xInx = 0, force eOutx = 0
        def eOutxUp2(mx, r, x, t):
            return mx.eOutx[r, x, t] <= mx.Mmax * mx.yx[r, x, t]

        @m.Constraint(m.X_, m.T)    # eOutxUp2 and eOutxLow1: force if xinx = 0, then eOutx = 0.
        def eOutxLow1(mx, r, x, t):
            return mx.eOutx[r, x, t] >= (
                    mx.x2e[r, x] * mx.xInxx[r, x, t]
                    + mx.x2eInt[r, x]
                    - mx.Mmax * (1 - mx.yx[r, x, t])
            )
...
#
#     def revise(self):
#         @self.m.Constraint(self.m.E_, self.m.T)  # x converted from e-th E2X converter
#         def xOuteC2(mx, r, e, t):
#             if mx.e2xInt[r, e] == 0:
#                 return pe.Constraint.Skip
#             else:
#                 if mx.eIne[r, e, t] != 0:
...
#                     return mx.xOute == 0
#
#         @self.m.Constraint(self.m.X_, self.m.T)  # electricity outflow from a X2E converter
#         def eOutxC2(mx, r, x, t):
#             if mx.x2eInt[r, x] == 0:
#                 return pe.Constraint.Skip
#             else:
#                 if mx.xInx[r, x, t] != 0:
