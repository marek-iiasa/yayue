...
class ParRep:     # representation of Pareto set
    ...
    def cube(self):  # define new cube (in ASF space), set A/R (in both ASF and A/R scales)
        ...
        self.cubes.append(Cube(s1, s2, mx_dis))
        asp = self.cubes[len(self.cubes) - 1].asp_vals
        res = self.cubes[len(self.cubes) - 1].res_vals
        return asp, res
