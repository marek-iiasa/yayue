...
class ParRep:     # representation of the Pareto set
    def __init__(self, wflow):         # initialize corners by regularized selfish solutions
        ...
        self.neigh = {}       # neighbors of the current solutions set {itr_id1: [itr_id2, dist]} (dominated excluded)
    ...
    def solDistr(self):     # distribution of distances between neighbor solutions, called by sizeLog
        ...
        is_grid = self.mc.opt('grid', False)
        ...
        if is_grid or is_mcube:
            if is_grid:
                ...
                raise Exception(f'ParRep::solDistr() - both grid and mCube options declared.')
                ...
                if self.mc.opt('mCube', False):
                    if self.wflow.cur_stage == 4:   # computing the PF (i.e., after Corners, neutral)
                        if self.neighSol is None:
                            ...
                            self.neighSol.addSol(None, True)    # ignore dominated sol, get next pair
                            pass
                    else:
                        pass    # do nothing before finishing Corners and neutral solution
                pass
    ...
    def mk_aCube1(self):  # find a pair of most distant neighbor solutions and define a cube.cand around them
