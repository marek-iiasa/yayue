...
class Grid:     # representation of the neighbors
    class aRay:
        ...
        def getSol(self, id):  # return the L^inf distance between the sol-pairs
            for s in self.sols:
                if s.itr_id == id:
                   return s
        ...
        def mkPairs(self):
            idBase = self.anch0   # base is the anchor with lowest id
            ...
            # sorting dist list
            dist.sort(key=itemgetter(2), reverse = True)    # sort in descending order
            # pair_lst = list(combinations(self.sols, 2))  # pairs of solutions
    ...
    def mkCand(self):
        ...
        for idItem, diff in self.neighCube.items():  # id consists of pt-pair and crit id
            ...
            # diff: max (over crit) of the achiev.-difference between neighbors (sorted by each criterion separately)
            # the diff might be smaller than the cube-size generated by the neighbors
    ...
    def selCand(self):
        ...
        for pair, item in self.cand.items():
